...
declare global {
  ...
  namespace Cypress {
    interface Chainable {
      ...
      setupWalletMocks(): Chainable<void>;
...
Cypress.Commands.add("setupWalletMocks", () => {
...
Cypress.Commands.add("setupNoWalletMocks", () => {
  Cypress.on("window:before:load", (win) => {
    console.log("Injecting StarkNet wallet provider mock...");

    win.starknet_argentX = null;
...
Cypress.Commands.add("login", () => {
  ...
  cy.get('[data-cy="wallet-address-btn"]', { timeout: 10000 }).should(
    "be.visible"
  );
});
