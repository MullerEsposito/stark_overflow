...
use stark_overflow::StarkOverflow::{
    IStarkOverflowSafeDispatcher, IStarkOverflowSafeDispatcherTrait, IStarkOverflowDispatcherTrait,
};
use stark_overflow::MockStarkToken::{IMockStarkTokenDispatcherTrait};
use super::common::{
    deploy_starkoverflow_contract, deploy_mock_stark_token, approve_as_spender, ask_question,
    create_forum, submit_answer,
};
...
fn test_deploy_mock_stark_token() {
    ...
    assert_eq!(
        stark_token_dispatcher.balance_of(asker), INITIAL_BALANCE, "Asker balance should be == 100",
    );
    assert_eq!(
        stark_token_dispatcher.balance_of(sponsor),
        INITIAL_BALANCE,
        "Sponsor balance should be == 100",
    );
}
...
fn it_should_be_able_to_create_a_forum() {
    ...
    let ownable_dispatcher = IOwnableDispatcher {
        contract_address: starkoverflow_contract_address,
    };
    ...
    let forum_id = starkoverflow_dispatcher
        .create_forum(forum_name.clone(), forum_icon_url.clone());
...
fn it_should_be_able_to_update_a_forum() {
    ...
    starkoverflow_dispatcher
        .update_forum(forum_id, new_forum_name.clone(), new_forum_icon_url.clone());
...
fn it_should_be_able_to_retrieve_all_not_deleted_forums() {
    ...
    let forum_id_2 = starkoverflow_dispatcher
        .create_forum(forum_name_2.clone(), forum_icon_url_2.clone());
...
fn it_should_be_able_to_ask_a_question() {
    ...
    let question_id = starkoverflow_dispatcher
        .ask_question(
            forum_id,
            title.clone(),
            description.clone(),
...
fn it_should_be_able_to_add_funds_to_a_question() {
    ...
    approve_as_spender(
        starkoverflow_contract_address, sponsor, stark_token_dispatcher, additionally_funds,
    );
...
fn it_should_be_able_to_mark_answer_as_correct() {
    ...
    assert_eq!(
        stark_token_dispatcher.balance_of(starkoverflow_contract_address),
        0,
        "Starkoverflow contract should initially have 0 balance",
    );
    ...
    let answer_id = submit_answer(
        starkoverflow_dispatcher, starkoverflow_contract_address, question_id, "Answer of test.",
    );
    ...
    let starkoverflow_contract_balance = stark_token_dispatcher
        .balance_of(starkoverflow_contract_address);
    assert_eq!(
        starkoverflow_contract_balance, 0, "Starkoverflow contract should finally have 0 balance",
    );
...
fn it_should_not_be_able_to_mark_a_non_existent_answer_as_correct() {
    ...
    let starkoverflow_safe_dispatcher = IStarkOverflowSafeDispatcher {
        contract_address: starkoverflow_contract_address,
    };
    ...
    match starkoverflow_safe_dispatcher.mark_answer_as_correct(question_id, inexistent_answer_id) {
        Result::Ok(_) => panic!(
            "It should not be able to mark a not related answer for a question",
        ),
        Result::Err(panic_data) => {
            let error_message = try_deserialize_bytearray_error(panic_data.span())
                .expect('wrong format');
            assert_eq!(
                error_message,
                "The specified answer does not exist for this question",
...
fn it_should_not_be_able_to_tag_an_answer_as_correct_but_the_owner() {
    ...
    match starkoverflow_safe_dispatcher.mark_answer_as_correct(question_id, answer_id) {
        Result::Ok(_) => panic!(
            "It should not be allowed no one than the question author to mark the question as correct",
        ),
        Result::Err(panic_data) => {
            let error_message = try_deserialize_bytearray_error(panic_data.span())
                .expect('wrong format');
            assert_eq!(
                error_message,
                "Only the author of the question can mark the answer as correct",
                "Wrong error message received",
            );
        },
    };
...
fn it_should_not_be_able_to_retrieve_answers_for_a_non_existent_question() {
    ...
    cheat_caller_address(
        starkoverflow_contract_address, ADDRESSES::ASKER.get(), CheatSpan::TargetCalls(1),
    );
    match starkoverflow_safe_dispatcher.get_answers(non_existent_question_id) {
        Result::Ok(_) => panic!(
            "It should not be able to retrieve answers for a non-existent question",
        ),
        Result::Err(panic_data) => {
            let error_message = try_deserialize_bytearray_error(panic_data.span())
                .expect('wrong format');
...
fn it_should_return_paginated_questions_for_a_forum() {
    ...
    let (questions_page1, total, has_next) = starkoverflow_dispatcher
        .get_questions(forum_id_1, 2, 1);
